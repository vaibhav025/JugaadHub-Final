...
import { X, ShieldCheck, CheckCircle2, Wallet, QrCode, Loader2, Copy } from "lucide-react";
...
export default function FinalPaymentModal({ rental, onClose }: { rental: any, onClose: () => void }) {
  ...
  const handleFinalPayment = async () => {
    ...
    try {
      ...
      await sendMessage(`✅ PAYMENT DONE!\n\nI have paid ₹${exactTotalPayable} for ${item.title}. My Handover OTP is: ${newOtp}`, rental.owner_id);

      // 🔥 FIX 3: Save OTP in state, show success, then slide to OTP screen
      setOtp(newOtp);
      setStep("success");
  ...
  return (
    <div 
      ...
      onClick={(e) => {
        // Sirf Pay step pe bahar click karne se close hoga, OTP screen par galti se band na ho jaye
        if (e.target === e.currentTarget && step === "pay") handleDismiss();
      }}
    >
          ...
          {step === "pay" && (
            <>
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                <button onClick={() => setMethod("wallet")} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${method==='wallet' ? 'border-[#004643] bg-[#004643]/5 shadow-sm' : 'border-[#004643]/10 bg-white hover:border-[#004643]/30'}`}>
                ...
                <button onClick={() => setMethod("upi")} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${method==='upi' ? 'border-[#004643] bg-[#004643]/5 shadow-sm' : 'border-[#004643]/10 bg-white hover:border-[#004643]/30'}`}>
                  <QrCode className={`w-6 h-6 ${method==='upi' ? 'text-[#004643]' : 'text-[#004643]/40'}`} /> 
                  <span className={`text-[10px] font-black ${method==='upi' ? 'text-[#004643]' : 'text-[#004643]/50'}`}>UPI QR</span>
                </button>
              </div>
              <button onClick={handleFinalPayment} className="w-full py-4 bg-[#004643] text-[#F0EDE5] rounded-2xl font-black shadow-lg hover:bg-[#004643]/90 transition-all active:scale-95">Pay & Get OTP</button>
            </>
          )}
          ...
          {step === "otp" && (
            <div className="py-4 flex flex-col items-center text-center animate-in slide-in-from-bottom-4 duration-300">
              ...
              <div className="bg-[#004643] rounded-3xl px-6 py-6 mb-6 w-full relative shadow-xl shadow-[#004643]/20">
                ...
                <button onClick={copyOtp} className="absolute top-3 right-3 p-2 rounded-xl bg-white/10 text-[#F0EDE5]/60 hover:text-[#F0EDE5] hover:bg-white/20 transition">
